...
def start_training(trainer: FeaturePredictorTrainer, train_loader: DataLoader, validation_loader: DataLoader,
    ...
    optimiser = optim.Adam(params=trainer.model.fc.parameters(), lr=learning_rate)
...
if __name__ == "__main__":
    ...
    trainer.load_model_from_file(device="cpu")
    trainer.model.eval()
    valid_images, valid_classes = next(iter(validation_loader))
    valid_images = valid_images[:10]
    predicted_classes: Tensor = output_to_predictions(model_output=trainer.model(valid_images[:10]))
